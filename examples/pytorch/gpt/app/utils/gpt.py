...
class GPTWeights(object):
    def __init__(self, head_num, size_per_head, layer_num, vocab_size, max_seq_len, tensor_para_size, pipeline_para_size,
        ...
        self.w.extend([torch.zeros(global_hidden_units, local_hidden_units * 3, dtype=str_type_map[
            self.inference_data_type])] * layer_num)   # self_kernel
        self.w.extend([torch.zeros(local_hidden_units * 3, dtype=str_type_map[
            self.inference_data_type])] * layer_num)   # self_bias
        ...
        self.w.extend([torch.zeros(global_hidden_units, local_inter_size, dtype=str_type_map[
            self.inference_data_type])] * layer_num)   # ffn_kernel1
        ...
        self.w.extend([torch.zeros(local_inter_size, global_hidden_units, dtype=str_type_map[
            self.inference_data_type])] * layer_num)   # ffn_kernel2
        self.w.extend([torch.zeros(global_hidden_units, dtype=str_type_map[
...
class GPT(nn.Module):
    ...
    def forward(self,
                ...
                request_id: int = None,	
                stream_tokens_pipe: int = None):
        print("<GPT.forward> enter")
